...
```Var(M) = 1 / (n - 1) * СУММА(для i от 1 до n) (m[i] - m̄)^2```

Где:
- ```M``` — рассматриваемая величина,
- ```m[i]``` — значение измерения под номером i,
- ```m̄``` — среднее значение всех измерений,
...
| Параметр               | Дисперсия                              | Стандартное отклонение                     |
|------------------------|----------------------------------------|--------------------------------------------|
| Определение            | Среднее значение квадратов отклонений | Квадратный корень из дисперсии             |
| Формула               | σ² = Σ(x_i - μ)² / N                 | σ = √(Σ(x_i - μ)² / N)                    |
| Единицы измерения      | Квадраты единиц данных                | Те же единицы, что и данные                |
| Интерпретация          | Менее наглядна, используется в теории  | Более интуитивно понятна, используется на практике |
| Пример (данные: [2,4,6,8,10]) | 8 (квадратные единицы)              | 2.83 (те же единицы, что и данные)         |
| Когда использовать     | Теоретические расчеты, моделирование   | Анализ данных, практическая интерпретация   |